...
def numbers_sum(num, num_sum):
    # Проверяем базовое значение, суммируем цифру
    if num // 10 == 0:
...
# Если четная цифра, суммируем с первым элементом, иначе суммируем со вторым
def sum_odd_or_even(num, num_sum):
    # Проверяем, четная ли цифра
    if num % 2:
        num_sum[1] += num  # Суммируем с нечетными
    else:
        num_sum[0] += num  # Суммируем с четными
